...
const log = [
  ...
  "[CHAT WINDOW TEXT] [Tue Feb  4 22:11:36] F attacks Arcane Archer : *hit* : (14 + 38 = 52)",
...
pub fn initiative_test() {
  ...
  let target = [True, False, False, False, False]
...
pub fn attack_test() {
  ...
  let target = [False, True, False, False, False]
  let results = log |> list.map(regexp.check(parser.regexp, _))
  should.equal(results, target)
}
...
pub fn aoo_test() {
  ...
  let target = [False, False, True, False, False]
  let results = log |> list.map(regexp.check(parser.regexp, _))
  should.equal(results, target)
}
...
pub fn damage_test() {
  ...
  let target = [False, False, False, True, False]
  let results = log |> list.map(regexp.check(parser.regexp, _))
  should.equal(results, target)
}
...
pub fn heal_test() {
  ...
  let target = [False, False, False, False, True]
  let results = log |> list.map(regexp.check(parser.regexp, _))
  should.equal(results, target)
}
